...
|---|------|--------|----------------|
| 1 | Fix MongoDB Docker configuration and replica set initialization | `pending` | `Fix MongoDB replica set startup by removing auth and using init container` |
| 2 | Update connection strings across codebase | `pending` | `Update connection strings to match auth-free MongoDB configuration` |
